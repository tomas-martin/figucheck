...
CREATE TABLE IF NOT EXISTS public.user_stickers (
    ...
    user_sync_code TEXT NOT NULL,
    sticker_number INT NOT NULL REFERENCES public.stickers(sticker_number),
    count INT DEFAULT 0,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()),
    UNIQUE(user_sync_code, sticker_number)
);

-- Habilitar RLS (Row Level Security) y dar acceso público para lectura y escritura por sync_code
ALTER TABLE public.teams ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.stickers ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.user_stickers ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Permitir lectura publica de equipos" ON public.teams FOR SELECT USING (true);
CREATE POLICY "Permitir lectura publica de figuritas" ON public.stickers FOR SELECT USING (true);
CREATE POLICY "Permitir operacion publica de progreso por sync_code" ON public.user_stickers FOR ALL USING (true) WITH CHECK (true);
